...
export type DiagramRoute = { kind: 'diagram'; id: string } | { kind: 'tracker'; key: string } | { kind: 'file'; id: string } | { kind: 'local-file'; id: string }
...
export function canonicalDiagramUrl(url: URL, base: URL): URL {
  ...
  const rootAlias = route.kind === 'diagram' && route.id === 'main'
    && !url.searchParams.has('diagram') && ['', 'index.html'].includes(url.pathname.slice(base.pathname.length))
  const path = rootAlias ? './' : route.kind === 'diagram' ? diagramUrl(route.id)
...
export function parseDiagramRoute(url: URL, base: URL = new URL('/', url)): DiagramRoute {
  ...
  if (path === '' || path === 'index.html') {
    if (url.searchParams.has('fileSession')) {
      ...
      if (id !== 'main' && isDiagramId(id)) return { kind: 'file', id }
    ...
    if (url.searchParams.has('localFile')) {
      ...
      if (id === '1' || id !== 'main' && isDiagramId(id)) return { kind: 'local-file', id: id! }
    ...
    if (file && (file[2] === '1' ? file[1] === 'local' : file[2] !== 'main' && isDiagramId(file[2]))) {
