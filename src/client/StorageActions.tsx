...
export function StorageActions({ mode, close, returnFocus, session, title, requestIdentity, navigate, openLocal }: {
  ...
  return <dialog ref={dialog} className="diagrams-dialog file-dialog confirmation-dialog" aria-labelledby="storage-action-heading"
    ...
    {transfer && <>
      ...
      {!canSaveDiskFile() && <p>Для переноса нужны HTTPS или локальный запуск в браузере с поддержкой прямой записи в файл. В этом режиме можно скачать копию, а затем отдельно удалить схему.</p>}
    </>}
