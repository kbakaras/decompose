...
test('redirect preserves query, another nested mount works and error recovery retains the prefix', async ({ page, request }) => {
  ...
  await page.goto(base + 'index.html?diagram=main')
  await ready(page)
  await expect(page).toHaveURL(base + 'diagram/main')
  ...
  await page.getByRole('link', { name: 'Вернуться к основной схеме' }).click()
...
test('canonical diagram paths accept old links without adding a history entry at either mount', async ({ page }) => {
  for (const base of [rootBase, prefixBase]) {
    ...
    await expect(page.getByRole('alert')).toContainText('Некорректная ссылка')
    await page.getByRole('link', { name: 'Вернуться к основной схеме' }).click()
    await ready(page); await expect(page).toHaveURL(base)
  }
})
