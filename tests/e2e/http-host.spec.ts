...
test('HTTP on a custom hostname supports UUIDs, same-origin API, collaboration and reload', async ({ page, browser }) => {
  ...
  await page.getByLabel('Поиск или название новой схемы', { exact: true }).fill('Схема по HTTP')
  ...
  try {
    ...
    expect((await download).suggestedFilename()).toBe('HTTP-4173.deco')
    await page.getByRole('button', { name: 'Схемы', exact: true }).click()
    await page.getByRole('button', { name: 'Перенести в файл', exact: true }).click()
    await expect(page.getByRole('button', { name: 'Перенести в файл', exact: true })).toBeDisabled()
    await expect(page.getByRole('dialog', { name: 'Перенести схему в файл' })).toContainText('HTTPS')
    await page.getByRole('button', { name: 'Отмена', exact: true }).click()
    await page.getByRole('button', { name: 'Схемы', exact: true }).click()
    await page.getByRole('tablist', { name: 'Раздел каталога' }).getByRole('tab', { name: 'Файлы', exact: true }).click()
    await page.getByRole('button', { name: 'Открыть файл на диске', exact: true }).click()
    await expect(page.getByRole('dialog')).toHaveCount(0)
