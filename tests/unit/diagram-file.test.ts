...
it('round-trips only the visible tree, order, soft breaks, statuses and settings', () => {
  ...
  commands.toggleStatus(a); commands.move(b, a, 0); commands.setTextAlign('center')
  ...
  const shape = (value: Y.Doc) => snapshotDiagram(value).nodes.map(node => ({ text: node.text, status: node.status, children: node.children.length }))
